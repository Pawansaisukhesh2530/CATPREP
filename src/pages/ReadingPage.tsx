...
export function ReadingPage({
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="grid gap-4 xl:grid-cols-3">
      <Card className="xl:col-span-3">
        <div className="flex flex-wrap items-center gap-3">
          <h3 className="text-sm font-semibold text-cyan-200">Live Articles</h3>
          <select
            value={topic}
      ...
      <Card className="xl:col-span-2 max-h-[78vh] overflow-hidden">
        <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
          <div>
            <h2 className="font-heading text-2xl text-white">{selectedArticle?.title ?? 'Select an article'}</h2>
            <p className="text-sm text-slate-400">Live Guardian content • {Math.max(1, Math.round(totalWords / 220))} min read • {totalWords} words</p>
          </div>
        ...
        <article className="max-h-[58vh] space-y-6 overflow-y-auto pr-2 text-slate-100" style={{ fontSize, lineHeight: 1.9 }}>
      ...
      <Card className="space-y-4">
        <h3 className="text-sm font-semibold text-cyan-200">Reflection Panel</h3>
